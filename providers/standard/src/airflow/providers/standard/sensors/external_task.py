...
class ExternalTaskSensor(BaseSensorOperator):
    ...
    *not* fail if the external task fails, but will continue to check the status
    ...
    @staticmethod
    def _serialize_dttm_filter_in_default_timezone(dttm_filter: Sequence[datetime.datetime]) -> str:
        return ",".join(
            timezone.coerce_datetime(dt, settings.TIMEZONE).in_timezone(settings.TIMEZONE).isoformat()
    ...
    def poke(self, context: Context) -> bool:
        ...
        display_dttm_filter = self._serialize_dttm_filter_in_default_timezone(dttm_filter)
        # Save as attribute - to be used by listeners
        self.external_dates_filter = serialized_dttm_filter

        if self.external_task_ids:
            self.log.info(
                "Poking for tasks %s in dag %s on logical_date %s (shown in default timezone: %s) ... ",
        ...
        if self.external_task_group_id:
            self.log.info(
                "Poking for task_group '%s' in dag '%s' on logical_date %s "
                "(shown in default timezone: %s) ... ",
        ...
        if self.external_dag_id and not self.external_task_group_id and not self.external_task_ids:
            self.log.info(
                "Poking for DAG '%s' on logical_date %s (shown in default timezone: %s) ... ",
                self.external_dag_id,
                serialized_dttm_filter,
                display_dttm_filter,
            )
